...
export function EventsList({ events, isFallback = false }: { events: any[], isFallback?: boolean }) {
  ...
  const hasMoreThan4 = events.length > 4;
  const hasMoreThan8 = events.length > 8;
    ...
    <>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-px bg-transparent sm:bg-arena-border">
        {events.map((e, i) => {
          ...
          const content = (
            <div className={`bg-arena-surface sm:bg-arena-bg p-5 h-full relative overflow-hidden group ${e.url ? "cursor-pointer" : "cursor-default"} flex flex-col justify-between rounded-lg border border-arena-border sm:rounded-none sm:border-0`}>
          ...
          return (
            <Reveal key={e._id || e.title} delay={(i % 4) * 0.08} className={`h-full ${hiddenClass}`}>
              {e.url ? (
